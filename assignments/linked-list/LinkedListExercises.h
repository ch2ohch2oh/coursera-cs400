...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if(!head_) {
    head_ = tail_ = newNode;
    // std::cout << "Insert an empty list" << std::endl;
  } else {
    Node * pos = head_;
    while(pos && newData > pos->data) {
      pos = pos->next;
    }
    
    if(!pos) {      // Insert at the tail
      // std::cout << "Insert at tail" << std::endl;
      newNode->prev = tail_;
      tail_->next = newNode;
      tail_ = newNode;
    } else if(pos == head_) {   // Insert at the head
      // std::cout << "Insert at head" << std::endl;
      newNode->next = head_;
      head_->prev = newNode;
      head_ = newNode;
    } else {        // Insert in the middle (right before pos)
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  Node * lnode = left.head_;
  Node * rnode = right.head_;
  while(lnode && rnode) {
    if(lnode->data < rnode->data) {
      merged.pushBack(lnode->data);
      lnode = lnode->next;
    } else {
      merged.pushBack(rnode->data);
      rnode = rnode->next;
    }
  }
  while(lnode) {
